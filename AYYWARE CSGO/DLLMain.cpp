...
#define felix -1496286547
#define felikstitanxpc 143117661
#define kubusmadhvh 945291704
#define xluq -1800809407
#define deathander 1213810804
#define alone 308279979
...
BOOL WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		if (dwSerial == me ||
			dwSerial == felix ||
			dwSerial == felikstitanxpc ||
			dwSerial == kubusmadhvh || 
			dwSerial == xluq ||
			dwSerial == deathander ||
			dwSerial == alone)
		{
		...
		{
			// when HWID rejected
			MessageBox(NULL, "Wrong HWID!", "PhantomWare Beta", MB_OK);
			exit(0);
			return TRUE;
		}
