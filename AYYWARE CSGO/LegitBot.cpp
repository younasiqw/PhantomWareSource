...
void CLegitBot::Move(CUserCmd *pCmd, bool& bSendPacket)
{
		...
		if (StartAim > 0)
		{
			...
			else
			{
				if (Aimtime > 0)
				{
					...
					if (!GUI.GetKeyState(Options::Menu.LegitBotTab.AimbotKeyBind.GetKey()) || Options::Menu.LegitBotTab.AimbotAutoFire.GetState())
					{
						CustomAimTime = 0;
						CustomAimStart = 0;
					}
	...
	if (Options::Menu.LegitBotTab.TriggerEnable.GetState() && ((Options::Menu.LegitBotTab.TriggerKeyPress.GetState() && GUI.GetKeyState(Options::Menu.LegitBotTab.TriggerKeyBind.GetKey()) || !Options::Menu.LegitBotTab.TriggerKeyPress.GetState())))
	{
		DoTrigger(pCmd);
	}
...
void CLegitBot::SyncWeaponSettings()
{
	...
	if (GameUtils::IsPistol(pWeapon))
	{
		...
		RecoilControl = Options::Menu.LegitBotTab.WeaponPistRecoil.GetValue();
	...
	else if (GameUtils::IsSniper(pWeapon))
	{
		...
		RecoilControl = Options::Menu.LegitBotTab.WeaponSnipRecoil.GetValue();
	...
	else if (GameUtils::IsMachinegun(pWeapon))
	{
		...
		RecoilControl = Options::Menu.LegitBotTab.WeaponMGRecoil.GetValue();
	...
	else if (GameUtils::IsShotgun(pWeapon))
	{
		...
		RecoilControl = Options::Menu.LegitBotTab.WeaponShotgunRecoil.GetValue();
	...
	else if (GameUtils::IsMP(pWeapon))
	{
		...
		Aimtime = Options::Menu.LegitBotTab.WeaponMpAimtime.GetValue() / 10;
		StartAim = Options::Menu.LegitBotTab.WeaoponMpStartAimtime.GetValue() / 10;
	}
	else
	{
...
void CLegitBot::DoTrigger(CUserCmd *pCmd)
{
	...
	if (!Options::Menu.LegitBotTab.TriggerTeammates.GetState()) {
